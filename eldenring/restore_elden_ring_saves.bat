...
REM echo on
REM when user gives the save number to restore, non-interactive mode
REM but here I don't know why, the !x! expansion needs to be used in filename
if not [%1]==[] (
    set /A counter2=1
    ...
    REM when reaching here, nothing matches; but in this case the script just run once because this usage intends to be non-interactive mode,
    REM so we don't show the menu
    echo Error, couldn't find save file index NO. %index%
    TIMEOUT /T 50
    REM exit
) else (
    goto :begin
